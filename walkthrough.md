...
Скиллы могут жить только в `.py` файлах под `implementations/` и быть помечены `@skill`.
Реестр автоматически обнаруживает такие функции без YAML-метаданных.

### Dev Mode

`Orchestrator.run_simple()` запускает скиллы без зависимостей (Postgres/Redis/Policy),
что ускоряет локальную разработку и отладку.

### Стабильные импорты

При загрузке скиллов реестр корректно инициализирует пакетную структуру
(`implementations`, подпакеты и `__init__.py`), поэтому относительные импорты
внутри `implementations/*` работают корректно.
